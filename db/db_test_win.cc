...
inline void RemoveTrailingNulls(std::string& str) {
  auto last_non_null_pos = str.find_last_not_of('\0');
  if (last_non_null_pos != std::string::npos) {
    str.resize(last_non_null_pos + 1);
  } else {
    str.clear();
  }
}
...
class DBTest : public testing::Test {
 public:
  ...
  DBTest() : env_(new SpecialEnv(Env::Default())), option_config_(kDefault) {
    ...
    std::wstring db_path = L"D:/Sevenli/²âÊÔ/leveldb";
    
    dbname_ = wstring_to_utf8(db_path);  // testing::TempDir() + "db_test";
    RemoveTrailingNulls(dbname_);
  ...
  Status TryReopen(Options* options) {
    ...
    auto status = DB::Open(opts, dbname_, &db_);
    if (!status.ok()) {
